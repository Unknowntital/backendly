...
import {
    Database, LogOut, ChevronDown, Users, BarChart3, Home, Menu
} from "lucide-react";
...
function MobileSidebar({ user, onLogout }) {
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="w-72 bg-[#08090C] border-r border-white/[0.06] p-5 flex flex-col">
                <Link to="/" className="flex items-center gap-2 mb-8" onClick={() => setOpen(false)}>
                    <div className="h-8 w-8 rounded-lg border border-teal-400/40 bg-black/40 flex items-center justify-center font-mono text-teal-300 text-sm font-bold">{"{}"}</div>
                    <span className="font-display font-bold text-lg tracking-tight text-white">Backendly</span>
                </Link>
                <nav className="space-y-1 text-sm" aria-label="Mobile nav">
                    {NAV.map(({ to, label, icon: Icon, end }) => (
                        <NavLink
                            key={to}
                            to={to}
    ...
    return (
        <div className="min-h-screen bg-[#08090C] text-white flex" data-testid="dashboard-page">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-white/[0.015]">
                    <MobileSidebar user={user} onLogout={handleLogout} />
                    <Link to="/" className="flex items-center gap-2">
                        <div className="h-7 w-7 rounded-lg border border-teal-400/40 bg-black/40 flex items-center justify-center font-mono text-teal-300 text-xs font-bold">{"{}"}</div>
                        <span className="font-display font-bold text-base tracking-tight text-white">Backendly</span>
                    </Link>
